...
2. **标签搜索**：单击左侧标签栏中的标签可包含该标签，双击则可排除，包含的标签显示为绿色，排除的标签显示为红色。点击已选中的标签可取消选择，支持多标签组合搜索，搜索完成后会自动更新可以进一步选择的标签，标签括号中的数字表示含有该标签的图片数量。
   ![标签搜索示例](./img/tag_search.png)
3. **文本搜索**：选择搜索字段（如作者、标题等）然后在顶部搜索栏输入进行全文搜索，支持模糊匹配。
   ![文本搜索示例](./img/text_search.png)
4. **筛选图片**：使用侧边栏的筛选条件和排序选项，快速找到目标图片
   ![筛选示例](./img/filter.png)
   ![排序示例](./img/sort_order.png)
